...
{
    # Import Common -ArgumentList:$boundParameters -silent:$silent
...
Set-Alias Get-Records Get-JsonFromFile -Scope Global
...
function Update-Record(
    ...
    # If no explicit ID provided, find the record to get its ID
    if ($id -le 0) {
        if ($query) {
            $list = Get-Records $storePath
            $toUpdate = $list | where $query | select -First 1
            
            if ($toUpdate) {
                $id = $toUpdate.Id
            }
            # If no matching record found, $id remains 0 and Add-Record will generate a new ID
        }
        # If no query provided and no ID, $id remains 0 and Add-Record will generate a new ID
    }
...
)
{
    ...
    # Ensure $list is a proper array for += operation
    $list = @($list)
    
    if ($id -le 0) {
        $maxId = 0
        foreach ($x in $list) {
            if ($null -ne $x -and $x.PSObject.Properties['Id']) {
                $v = [int]$x.Id
                if ($v -gt $maxId) { $maxId = $v }
            }
        }
        $id = if ($maxId -gt 0) { $maxId + 1 } else { 1 }
    }
